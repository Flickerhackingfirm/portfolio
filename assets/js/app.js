/* -----------------------------------------------
/* How to use? : Check the GitHub README
/* ----------------------------------------------- */

/* To load a config file (particles.json) you need to host this demo (MAMP/WAMP/local)... */
/*
particlesJS.load('particles-js', 'particles.json', function() {
  console.log('particles.js loaded - callback');
});
*/

// Create a MutationObserver to watch for theme changes
const observer = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    if (mutation.attributeName === 'data-theme') {
      const newTheme = document.documentElement.getAttribute('data-theme');
      updateParticleColors(newTheme);
    }
  });
});

// Start observing the document element for attribute changes
observer.observe(document.documentElement, {
  attributes: true,
  attributeFilter: ['data-theme']
});

// Initial theme check and color update
const initialTheme = document.documentElement.getAttribute('data-theme');
if (initialTheme) {
  updateParticleColors(initialTheme);
}

/* Otherwise just put the config content (json): */

// Function to update particle colors based on theme
function updateParticleColors(theme) {
  const particleColor = theme === 'dark' ? '#ffffff' : '#000000';
  const particleOpacity = theme === 'dark' ? 0.8 : 0.5;
  const lineOpacity = theme === 'dark' ? 0.6 : 0.4;
  
  if (window.pJSDom && window.pJSDom[0]) {
    const pJS = window.pJSDom[0].pJS;
    const particles = pJS.particles;
    
    // Update colors and opacity settings
    particles.color.value = particleColor;
    particles.line_linked.color = particleColor;
    particles.opacity.value = particleOpacity;
    particles.line_linked.opacity = lineOpacity;
    
    // Update existing particles
    particles.array.forEach(particle => {
      particle.color = particleColor;
    });
    
    // Update canvas context properties
    if (pJS.canvas.ctx) {
      pJS.canvas.ctx.strokeStyle = particleColor;
    }
    
    // Force particles refresh
    pJS.fn.particlesRefresh();
  }
}

// Initialize particles with theme-aware colors
const currentTheme = document.documentElement.getAttribute('data-theme') || 'light';
const initialColor = currentTheme === 'dark' ? '#ffffff' : '#000000';

particlesJS('particles-js', {
  "particles": {
    "number": {
      "value": 80,
      "density": {
        "enable": true,
        "value_area": 800
      }
    },
    "color": {
      "value": initialColor
    },
    "shape": {
      "type": "circle",
      "stroke": {
        "width": 0,
        "color": initialColor
      },
      "polygon": {
        "nb_sides": 5
      },
      "image": {
        "src": "img/github.svg",
        "width": 100,
        "height": 100
      }
    },
    "opacity": {
      "value": currentTheme === 'dark' ? 0.8 : 0.5,
      "random": false,
      "anim": {
        "enable": false,
        "speed": 1,
        "opacity_min": 0.1,
        "sync": false
      }
    },
    "size": {
      "value": 5,
      "random": true,
      "anim": {
        "enable": false,
        "speed": 40,
        "size_min": 0.1,
        "sync": false
      }
    },
    "line_linked": {
      "enable": true,
      "distance": 150,
      "color": initialColor,
      "opacity": currentTheme === 'dark' ? 0.6 : 0.4,
      "width": 1
    },
    "move": {
      "enable": true,
      "speed": 6,
      "direction": "none",
      "random": false,
      "straight": false,
      "out_mode": "out",
      "attract": {
        "enable": false,
        "rotateX": 600,
        "rotateY": 1200
      }
    }
  },
  "interactivity": {
    "detect_on": "canvas",
    "events": {
      "onhover": {
        "enable": true,
        "mode": "repulse"
      },
      "onclick": {
        "enable": true,
        "mode": "push"
      },
      "resize": true
    },
    "modes": {
      "grab": {
        "distance": 400,
        "line_linked": {
          "opacity": 1
        }
      },
      "bubble": {
        "distance": 400,
        "size": 40,
        "duration": 2,
        "opacity": 8,
        "speed": 3
      },
      "repulse": {
        "distance": 200
      },
      "push": {
        "particles_nb": 4
      },
      "remove": {
        "particles_nb": 2
      }
    }
  },
  "retina_detect": true
});